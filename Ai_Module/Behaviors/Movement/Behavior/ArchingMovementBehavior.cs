...
{
    /// <summary>
    /// Moves the vehicle along an arcing path based on configurable
    /// rotation and radius parameters.
    /// </summary>
    public class ArchingMovementBehavior : IMovementBehavior
    {
        public int CompletedCycles => Mathf.FloorToInt(progress/targetDistanceInRadians);
        private float progress = 0f;
        private float targetDistanceInRadians = Mathf.PI;
        ...
        public ArchingMovementBehavior()
        {
            time = 0f;
        }
        ...
        public void Execute(IWorldObject worldObject)
        {
            ...
            progress = time * rotationRate * 2 * Mathf.PI;
            var angle = offset + progress + startRadian;
        ...
        {
            if (data is ArchingMovementBehaviorConfig config)
            {
                rotationRate = config.rotationRate;
                arcRatioX = config.arcRatioX;
                arcRatioY = config.arcRatioY;
                clockwise = config.clockwise;
                startRadian = (clockwise ? -1 : 1) * config.startDegree * Mathf.Deg2Rad;
                targetDistanceInRadians = config.distanceInDegrees * Mathf.Deg2Rad;
            }
            else
            {
